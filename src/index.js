    ...
    render(){
        ...
        return (
            <View style={{display: 'flex',justifyContent: 'space-between', height: '100%', width: '100%'}}>
                <Header open={this.toggle} clear={clear} difficulty={difficulty} onChange={this.setDifficulty} mines={mines} />
                <MineSweeper
                    rows={rows}
                    columns={columns}
                    mines={mines}
                ...
                />
                {picker && <Picker open={this.toggle} difficulty={difficulty} onChange={this.setDifficulty} />}
            </View>
        );
    }
}