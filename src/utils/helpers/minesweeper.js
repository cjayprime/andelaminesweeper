export default class MineSweeper {
    ...
    expand = (x, y, allTiles) => {
        ...
        const rows = this.rows;
        const columns = this.columns;
        if (x < 0 || x > columns - 1 || y < 0 || y > rows - 1) return tiles;
        ...
        if (tiles[x][y].value === 0 && tiles[x][y].display !== 'show') {
            ...
            this.surrounding.map(coord => {
                ...
                if (newX < 0 || newX > columns - 1 || newY < 0 || newY > rows - 1) return;
                if (tiles[newX][newY].value === 'mine') return; // Stop once a mine is detected
