...
import {shovel, flag as FLAGICON, cancel} from '../../assets/images';
...
export default class Board extends Component {
  ...
  render() {
    ...
    const paddingTop =
      (windowHeight - Constants.HEADER_HEIGHT - rows * tileWidth) / 2;
    const position = {
    ...
    return (
      <>
        ...
        <View
          ...
          <TouchableOpacity
            style={Style.board.popoverShovel}
            activeOpacity={0.6}
            onPress={() => this.act('shovel')}>
            <Image source={shovel} style={Style.board.popoverImage} />
          </TouchableOpacity>
          <TouchableOpacity
            style={Style.board.popoverFlag}
            activeOpacity={0.6}
            onPress={() => this.act('flag')}>
            <Image source={FLAGICON} style={Style.board.popoverImage} />
          </TouchableOpacity>
        </View>
        {(won || lost) && (
          <Modal
            animate={true}
            onClose={() => this.setState({won: false, lost: false})}>
            <View style={Style.board.modalContainer}>
              <Text style={Style.board.modalContainerText}>
                {'You ' + (won ? 'Win' : 'Lose')}
              </Text>
            </View>
            <View style={[Style.board.retryContainer]}>
              <Button
                label={'Retry'}
                onPress={() => this.setState({won: false, lost: false}, reset)}
              />
            </View>
          </Modal>
        )}
