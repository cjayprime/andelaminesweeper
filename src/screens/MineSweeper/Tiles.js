...
export default class Tiles extends Component{
    ...
    restart = () => {
        ...
        for(var i = 0; i < rows; i++){
            for(var j = 0; j < columns; j++){
                (function(j, i){
                    if(tiles[j][i].value === 'mine'){
                        tiles = this.minesweeper.reveal(j, i, tiles);
                    }
                }).apply(this, [j, i]);
            }
        }
    ...
    dig = (x, y) => {
        ...
        if(tiles[x][y].flag === false){
            ...
            this.setState({
                ...
                for(var i = 0; i < rows; i++){
                    for(var j = 0; j < columns; j++){
                        if(newTiles[j][i].display !== 'show' && newTiles[j][i].value !== 'mine'){
                            unRevealedTiles++;
                        }
                    }
                }
    ...
    render(){
        ...
        for(var i = 0; i < rows; i++){
            for(var j = 0; j < columns; j++){
                if(typeof tiles[j] !== 'undefined' && typeof tiles[j][i] !== 'undefined'){
                    (function(j, i){
                        if(j !== 0 && columns % 2 === 0){
                            color = !color;
                        }else if(columns % 2 !== 0){
                        ...
                        const isSet = tiles[j][i].value === 'mine'
                                        ?
                                        false
                                        :
                                        tiles[j][i].display === 'show'
                        //
                        board.push(
                            <TouchableWithoutFeedback
                                key={j + ' - ' + i}
                                onPress={() => this.handleClick(j, i)}
                            >
                                <View
                                    ref={view => {
                                        this.components[j][i] = view;
                                    }}
                                    style={[Style.tiles.container, {
                                        width: size,
                                        height: size,
                                        borderWidth: top >= 0 && left >= 0 && j === x && i === y ? 3 : 0,
                                ...
                                >
                                    {
                                        ...
                                        :   tiles[j][i].display === 'show' &&
                                            tiles[j][i].value === 'mine'
                                            ?   <Image source={mine} style={Style.tiles.image} />
                                            :
                                                tiles[j][i].display === 'show' &&
                                                tiles[j][i].value > 0 &&
                                                <Text style={Style.tiles.text}>
                                                    {
                                                        tiles[j][i].value
                                                    }
                                                </Text>
